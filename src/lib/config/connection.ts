...
export function encryptSecret(value: string): string { const key = encryptionKey(); if (!key) throw new Error('Encrypted secret storage is not configured.'); const iv = randomBytes(12); const cipher = createCipheriv('aes-256-gcm', key, iv); const ciphertext = Buffer.concat([cipher.update(value, 'utf8'), cipher.final()]); return `${iv.toString('base64url')}.${cipher.getAuthTag().toString('base64url')}.${ciphertext.toString('base64url')}`; }
export function decryptSecret(value: string): string | null { const key = encryptionKey(); if (!key) return null; try { const [iv, tag, ciphertext] = value.split('.'); if (!iv || !tag || !ciphertext) return null; const decipher = createDecipheriv('aes-256-gcm', key, Buffer.from(iv, 'base64url')); decipher.setAuthTag(Buffer.from(tag, 'base64url')); return Buffer.concat([decipher.update(Buffer.from(ciphertext, 'base64url')), decipher.final()]).toString('utf8'); } catch { return null; } }
